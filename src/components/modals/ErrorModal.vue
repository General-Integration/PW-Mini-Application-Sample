<template>
    <teleport to="body">
        <transition>
            <div v-show="useError.show" class="fixed z-50 inset-0 bg-black bg-opacity-50 flex items-start justify-center items-center min-h-screen text-center" @click="backdropDismiss($event)" data-dismiss="modal">
                <div class="bg-white rounded-md shadow-xl w-4/5 pt-4">
                    ...
                    <footer class="flex justify-center items-center p-4 border-t-2 border-gray-100">
                        <a class="font-bold text-lg text-cyan-500" @click="buttonDismiss($event)" data-dismiss="modal">Dismiss</a>
                    </footer>
                </div>
            </div>
        </transition>
